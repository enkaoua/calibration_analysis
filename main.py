...
def hand_eye_table(size_chess,image_pths,board, intrinsics_pth,distortion_pth, min_num_corners=6, waiting_time=1, return_corners=True):
    if return_corners:
        updated_image_pths_endo, min_num_corners_endo, T_all_endo, imgPoints_endo, objPoints_endo =    detect_charuco_board_pose_images( board, image_pths,intrinsics_pth, distortion_pth,return_corners=return_corners, min_num_corners=min_num_corners, waiting_time=waiting_time)
        updated_image_pths_rs, min_num_corners_rs, T_all_rs, imgPoints_rs, objPoints_rs =    detect_charuco_board_pose_images( board, image_pths,intrinsics_pth, distortion_pth,return_corners=return_corners, min_num_corners=min_num_corners, waiting_time=waiting_time)
    
    else:
        updated_image_pths, min_num_corners, T_all  =     detect_charuco_board_pose_images( board, image_pths,intrinsics_pth, distortion_pth,return_corners=return_corners, min_num_corners=min_num_corners, waiting_time=waiting_time)

        # convert updated image paths- split path to get: [image number, pose number, degree number, going forward or backward]
    # convert to pandas dataframe
    data = {'paths_endo':updated_image_pths_endo, 'imgPoints_endo':imgPoints_endo, 'objPoints_endo':objPoints_endo, 'T_endo':T_all_endo,
            'paths_rs':updated_image_pths_rs, 'imgPoints_rs':imgPoints_rs, 'objPoints_rs':objPoints_rs, 'T_endo':T_all_rs}
    data_df = pd.DataFrame(data=data)
    ...
    data_df.to_pickle(f'results/transform_data/{size_chess}_corner_data.pkl')
    info_df.to_csv(f'results/transform_data/{size_chess}_corner_info.csv')
    return data_df, info_df
...
def main_hand_eye():
    ...
    for size_chess in chess_sizes:
        ...
        data_df, info_df = hand_eye_table(size_chess, camera,image_pths,board, intrinsics_pth,distortion_pth, min_num_corners=6, waiting_time=1, return_corners=True)
...
def main_intrinsics(): 
    ...
    table_pth = f'results/raw_corner_data/{rec_data}'
    filtered_table_pth = f'results/split_data/{rec_data}'
    analysis_results_pth = f'results/calibration_analysis/{rec_analysis}'
    create_folders([table_pth, filtered_table_pth, analysis_results_pth])
    

    chess_sizes = [20, 15, 25, 30] # 15, 20, 25, 30
        ...
        for size_chess in chess_sizes:
            ...
            if os.path.isfile(calibration_analysis_results_save_pth):
                ...
                perform_analysis(camera, size_chess, 
                                ...
                                visualise_reprojection_error=visualise_reprojection_error, waitTime=1,
