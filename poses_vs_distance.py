...
def main(table_pth='results/hand_eye/raw_corner_data/MC_None_PC_None', 
    ...
    for camera in cameras:
        ...
        for chess_size in chess_sizes:
            ...
            distances = data_df_chess_size[f'T_z{extension}'].unique()
            ...
            for pose in poses:
                ...
                for distance in distances:
                    ...
                    if len(data_df_filtered_for_distance) < 5:
                ...
                if len(distances_lst) < 3:
                    print(f'skipping pose {pose} because not enough data')
                    continue
                    
                results_combined = pd.concat(results_iteration, axis=0)
                # save results for this pose and angle
                results_combined.to_pickle(
                    f'{distance_analysis}/results_P{pose}_distance{distance}.pkl')

                
                # plot reprojection error vs pose
                plt.plot(distances_lst, reprojection_errors, label=f'{pose}')
                # plot shaded plot of Q1 and Q3
                #plt.fill_between(distances_lst, Q1s, Q3s, alpha=0.2)
                #plt.plot(distances_lst, Q1s, color='blue')
                #plt.plot(distances_lst, Q3s, color='blue')
            
            plt.legend()
            plt.xlabel('Pose')
            plt.ylabel('Reprojection Error')
            plt.title(f'Reprojection Error vs Pose for {chess_size}mm chessboard for {camera} camera')
            plt.savefig(f'reprojection_error_vs_pose_{camera}_{chess_size}.png')
            plt.show()
